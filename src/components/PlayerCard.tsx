...
export function PlayerCard({ avatar, nickname, skill_level, countryFlag, status, epochString, match_id }: PlayerCardProps) {

  const ElapsedTime: React.FC<{ elapsedTime: string }> = ({ elapsedTime }) => {
    const [elapsed, setElapsed] = useState("");

    useEffect(() => {
      const update = () => {
        setElapsed(getElapsedTime(elapsedTime));
      };
    ...
    <Card className={`flex flex-row items-center bg-[#111] rounded-xl p-4 w-48 shadow-lg h-70 border-2 
      ...
      <Flex align="center" direction="column" gap="3">
        <Flex align="center" justify="end" direction="row" gap="3">
          <ElapsedTime elapsedTime={epochString} />
